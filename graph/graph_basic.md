...
<img src="./images/linkedlist_tree_graph.png" alt="链表、树、图之间的关系" style="zoom:50%;" />
...
- 边相对较少的图称为**稀疏图**（sparse graph），通常以 $|E|<|V|\log|V|$ 作为判断标准
- 缺少的边相对较少的图称为**密集图**（dense graph）
- 有向加权图（directed weighted graph）有时也称为网络（network）
...
- **路径**（path）：由边顺序连接的一系列顶点。在图 9-4 中，边序列 1-5-2-4 是顶点 1 到顶点 4 的一条路径。
  - 自环（self-loop）, 路径包含指向自身的 edge, 如 (v,v)
  - **简单路径**（simple-path）是没有重复顶点的路径（第一个和最后一个顶点可以相同）
  - 连接同一对顶点的两条边，称为**平行边**（parallel edge）。
  - 包含平行边的 graph 称为多重图（multigraph），没有平行边和自环的 graph 称为简单图（simple-graph）。
  - cycle：是一条至少含有一条边
  - simple-cycle：除了起点和终点，不包含重复顶点和边的 cycle。
  - cycle，对有向图，要求path 长度至少为 1，且 $v_1=v_N$；对无向图，则要求所有 edge 不同，例如，(u,v,u) 在无向图中就不属于 cycle，因为 (u,v) 和 (v,u) 属于同一个 edge；在有向图中则属于 cycle。
  - 有向无环图，简称 DAG
  - path 或 cycle 的长度定义为包含的边数
- 度（degree）：一个顶点拥有的边数。对于有向图，**入度**（in-degree）表示有多少条边指向该顶点，**出度**（out-degree）表示有多少条边从该顶点指出。
- tree 是无环连通图。互不相连的树组成的结合称为森林。
- 连通图的生成树是它的一个子图，它包含图中所有顶点且是一棵树
- 图的生成树森林是它的所有连通子图的生成树的集合

包含 V 个节点的图 G 满足以下 5 个条件之一，它就是一棵树：

- G 有 V-1 条边且不含环
- G 有 V-1 条边且是连通的
- G 是连通的，但删除任意一条边都会使它不再连通
- G 是无环的，但添加任何一条边都会产生一条环
- G 中的任意一对顶点之间进存在一条简单路径

二分图（bipartite graph）：可以将节点分为两部分，每条边所连接的两个顶点都属于不同的部分。如下图，红色是一个集合，黑色是另一个集合。

<img src="./images/image-20241202112634923.png" alt="image-20241202112634923" style="zoom:50%;" />

...
后面会介绍一个 $O(|V|^3)$ 算法解决 weighted-graph 的这个问题。对 dense-graph，这与运行简单 Dijkstra 算法 |V| 次的界限相同，但循环更紧密，因此专门的 all-pairs 算法在实践中更快。在稀疏图上，运行 $|V|$ 次 使用优先队列实现的 Dijkstra 算法更快。
